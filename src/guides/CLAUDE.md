...
4. **Gotcha-First** → 스킬 실행 중 오류 발생 시 **우회/해결 전에** 해당 SKILL.md `## Gotchas`에 근본 원인을 먼저 기록. 우회에 성공하면 기록 동기가 사라지고, 다음 세션에서 같은 오류를 반복한다.
5. **Parallel-First** → 작업 시작 전 병렬화 가능 여부를 먼저 검토. 독립 단위로 분해 → 의존 관계 없는 단위는 Agent 병렬 launch → 의존 관계 있는 단위만 순차 실행. 단일 작업 내에서도 독립적 tool call은 항상 병렬 호출.
...
bash에서 `claude -p`를 반복 호출하는 무인 자율 코딩. `ralph-loop --project-dir /path --task-plan criteria.md`

- 매 반복 fresh context + 파일시스템 상태 인수인계
- 수렴 아키텍처: task_plan 기준 → 라운드 반복 → 파일별 품질 수렴
- scope-state.json/progress.jsonl/git으로 상태 관리
