...
> *CC 소스맵 분석에서 차용: "판단 불가 시 거부가 아닌 사용자 질문으로 폴백"*

Hook이 차단(exit 2)할 때는 **수정 방법을 함께 안내**한다. 단순 거부는 에이전트를 막다른 골목에 넣는다.

```
[3단계 대응]
Critical (보안/시크릿)  → 무조건 차단 (exit 2), 대안 없음
Important (포맷/규칙)   → 차단 (exit 2) + 구체적 수정 가이드
Preference (스타일)     → 경고만 (exit 0), 진행 허용
```

예시 — commit-format-check.sh:
- 타입 프리픽스 누락 → **차단** + "허용 타입: feat, fix, ..." 안내 (Important)
- 한글 누락 → **경고만** + 진행 허용 (Preference)
