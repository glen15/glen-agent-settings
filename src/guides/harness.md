...
작업 요청 → Skill로 해결 가능한가?
  ...
  └─ NO → 외부 실시간 데이터가 필요한가?
       ├─ YES → MCP 사용 (최소 범위)
       └─ NO → CLI 도구 (gws, gh, browser-use, supabase 등)
```
...
4. CLI 도구가 있으면 MCP보다 CLI 우선: `gws > MCP Gmail/Calendar`, `gh > GitHub MCP`, `browser-use > firecrawl` (인터랙션/인증 필요 시)
