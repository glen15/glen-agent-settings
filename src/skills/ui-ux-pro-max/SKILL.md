---
name: ui-ux-pro-max
description: "UI/UX 디자인 시스템 생성 및 구현 가이드. 디자인 시스템, 컬러 팔레트, 폰트 페어링, 스타일 추천, Pre-Delivery 체크리스트 제공. UI/UX 설계·구현·리뷰 요청 시 사용."
user_invocable: true
argument-hint: <"제품유형 키워드"> [--design-system] [--domain style|color|typography|ux]
---
...
3. **다크 모드만 구현** — 라이트 모드 대비도 확인. `bg-white/10` 같은 투명도는 라이트에서 안 보인다.
4. **hover에 scale 사용** — 레이아웃 시프트를 유발. `color`, `opacity`, `shadow` 트랜지션 사용.
5. **cursor-pointer 누락** — 클릭 가능한 모든 요소에 `cursor-pointer` 필수.
