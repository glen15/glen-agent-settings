...
description: "Stitch 화면을 모듈식 React/Vite 컴포넌트로 변환. AST 기반 검증으로 타입 안전성 보장. /stitch-react-components — 디자인→코드 변환 시 사용."
user_invocable: true
argument-hint: [--screen-id ID] [--output-dir ./src/components]
allowed-tools:
  - "stitch*:*"
  - "Bash"
  - "Read"
