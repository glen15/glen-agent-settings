...
유의미한 작업(기능 추가, 버그 수정, 리팩토링 등)을 시작할 때 자동으로 추적한다.

- **작업 시작**: `/begin "작업명"` — 5개 추적 문서(plan/progress/context/failures/contract) 자동 생성
- **작업 중**: progress.md, context.md, failures.md를 단계 완료·발견·실패 시마다 업데이트
- **작업 완료**: `/done` — 커밋 + nxtflow 태스크 처리를 한 번에 수행

사용자가 명시적으로 지시하지 않아도, 유의미한 작업 시작 시 스스로 `/begin`을, 완료 시 `/done`을 사용한다.
단순 질문 응답, 파일 조회 등 추적이 불필요한 작업에는 사용하지 않는다.
