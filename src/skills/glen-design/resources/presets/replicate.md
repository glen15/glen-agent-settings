...
The design personality is defined by two extreme choices: **massive display typography** (up to 128px) using the custom Space Grotesk face, and **exclusively pill-shaped geometry** (9999px radius on everything). The display font is thick, bold, and confident — its heavy weight at enormous sizes creates text that feels like it's shouting with joy rather than whispering authority. Combined with Inter for body text (a clean geometric sans) and JetBrains Mono for code, the system serves developers who want power and playfulness in equal measure.
...
- Massive display typography (128px) in heavy Space Grotesk
...
### Font Family
- **Display**: `Space Grotesk`, with fallbacks: `ui-sans-serif, system-ui`
- **Body / UI**: `Inter`, with fallbacks: `ui-sans-serif, system-ui`
...
|------|------|------|--------|-------------|----------------|-------|
| Display Mega | Space Grotesk | 128px (8rem) | 700 | 1.00 (tight) | normal | The maximum: closing manifesto |
| Display / Hero | Space Grotesk | 72px (4.5rem) | 700 | 1.00 (tight) | -1.8px | Hero section headline |
| Section Heading | Space Grotesk | 48px (3rem) | 400–700 | 1.00 (tight) | normal | Feature section titles |
| Sub-heading | Space Grotesk | 30px (1.88rem) | 600 | 1.20 (tight) | normal | Card headings |
| Sub-heading Sans | Inter | 38.4px (2.4rem) | 400 | 0.83 (ultra-tight) | normal | Large body headings |
| Feature Title | Inter / Space Grotesk | 18px (1.13rem) | 600 | 1.56 | normal | Small section titles, labels |
| Body Large | Inter | 20px (1.25rem) | 400 | 1.40 | normal | Intro paragraphs |
| Body / Button | Inter | 16–18px (1–1.13rem) | 400–600 | 1.50–1.56 | normal | Standard text, buttons |
| Caption | Inter | 14px (0.88rem) | 400–600 | 1.43 | -0.35px to normal | Metadata, descriptions |
| Small / Tag | Inter | 12px (0.75rem) | 400 | 1.33 | normal | Tags (lowercase transform) |
...
### Principles
- **Heavy display, light body**: Space Grotesk at 700 weight creates thundering headlines, while Inter at 400 handles body text with quiet efficiency. The contrast is extreme and intentional.
...
- **Lowercase tags**: 12px Inter uses `text-transform: lowercase` — an unusual choice that creates a casual, developer-friendly vibe.
- **Weight 600 as emphasis**: When Inter needs emphasis, it uses 600 (semibold) — never bold (700), which is reserved for Space Grotesk display text.
...
- Use Space Grotesk at weight 700 for display text — go big (72px+) or go home
...
- Keep body text in Inter at 400–600 weight
...
- Don't use light/thin font weights on Space Grotesk — 600–700 is the range
...
- "Create a hero section with a vibrant orange-red-magenta gradient background. Headline at 72px Space Grotesk weight 700, white text, -1.8px letter-spacing. Include a dark pill CTA button and a white outlined pill button."
- "Design a model card with pill-shaped (9999px) image container, model name at 16px Inter weight 600, run count at 14px in Medium Gray. Border: 1px solid #202020."
- "Build a status badge: pill-shaped (9999px), Status Green (#2b9a66) background, white text at 14px Inter."
- "Create a manifesto section on Replicate Dark (#202020) with 'Imagine what you can build.' at 128px Space Grotesk weight 700, white text. Embed small AI-generated images between the words."
...
5. Use Inter for body, Space Grotesk for display, JetBrains Mono for code
