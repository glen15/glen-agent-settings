...
The typography is anchored by a custom brand font (Inter, loaded as `__Inter_96f0ca`) that carries substantial weight — literally. Headings use 600–700 weights with tight line-heights (1.17–1.19), creating dense, authoritative text blocks that communicate enterprise confidence. The hero headline at 82px weight 600 with OpenType `"kern"` enabled is not decorative — it's infrastructure-grade typography.
...
- Custom Inter font with 600–700 weights and `"kern"` feature
...
### Font Families
- **Primary Brand**: `__Inter_96f0ca` (Inter), with fallback: `__Inter Fallback_96f0ca`
...
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Inter | 82px (5.13rem) | 600 | 1.17 (tight) | normal | `"kern"` enabled |
| Section Heading | Inter | 52px (3.25rem) | 600 | 1.19 (tight) | normal | `"kern"` enabled |
| Feature Heading | Inter | 42px (2.63rem) | 700 | 1.19 (tight) | -0.42px | Negative tracking |
| Sub-heading | Inter | 34px (2.13rem) | 600–700 | 1.18 (tight) | normal | Feature blocks |
| Card Title | Inter | 26px (1.63rem) | 700 | 1.19 (tight) | normal | Card and panel headings |
| Small Title | Inter | 19px (1.19rem) | 700 | 1.21 (tight) | normal | Compact headings |
| Body Emphasis | Inter | 17px (1.06rem) | 600–700 | 1.18–1.35 | normal | Bold body text |
...
| Uppercase Label | Inter | 13px (0.81rem) | 600 | 1.69 (relaxed) | 1.3px | `text-transform: uppercase` |

### Principles
- **Brand/System split**: Inter for headings and brand-critical text; system-ui for body, navigation, and functional text. The brand font carries the weight, system-ui carries the words.
- **Kern always on**: All Inter text enables OpenType `"kern"` — letterfitting is non-negotiable.
...
- **Uppercase labels as wayfinding**: 13px uppercase with 1.3px letter-spacing serves as the systematic category/section marker — always Inter weight 600.
...
### Do
- Use Inter for headings and brand text, system-ui for body and UI text
- Enable `"kern"` on all Inter text
...
- Don't use Inter for small body text — it's designed for 17px+ heading use
...
- "Create a hero on dark background (#15181e). Headline at 82px Inter weight 600, line-height 1.17, kern enabled, white text. Sub-text at 20px system-ui weight 400, line-height 1.50, #d5d7db text. Two buttons: primary dark (#15181e, 5px radius, 9px 15px padding) and secondary white (#ffffff, 4px radius, 8px 12px padding)."
- "Design a product card: white background, 8px radius, dual-layer shadow at rgba(97,104,117,0.05). Title at 26px Inter weight 700, body at 16px system-ui weight 400 line-height 1.63."
- "Build an uppercase section label: 13px Inter weight 600, line-height 1.69, letter-spacing 1.3px, text-transform uppercase, #656a76 color."
...
2. Inter for headings only (17px+), system-ui for everything else
