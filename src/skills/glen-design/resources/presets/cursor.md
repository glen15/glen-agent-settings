...
The custom Inter font is the typographic signature -- a gothic sans-serif with aggressive negative letter-spacing at display sizes (-2.16px at 72px) that creates a compressed, engineered feel. As a secondary voice, the Fraunces serif font (with OpenType `"cswh"` contextual swash alternates) provides literary counterpoint for body copy and editorial passages. The monospace voice comes from JetBrains Mono, a refined coding font that connects the marketing site to Cursor's core identity as a code editor. This three-font system (gothic display, serif body, mono code) gives Cursor one of the most typographically rich palettes in developer tooling.
...
**Key Characteristics:**
- Inter with aggressive negative letter-spacing (-2.16px at 72px, -0.72px at 36px) for compressed display headings
- Fraunces serif for body text with OpenType `"cswh"` (contextual swash alternates)
...
### Font Family
- **Display/Headlines**: `Inter`, with fallbacks: `Inter Fallback, system-ui, Helvetica Neue, Helvetica, Arial`
- **Body/Editorial**: `Fraunces`, with fallbacks: `Iowan Old Style, Palatino Linotype, URW Palladio L, P052, ui-serif, Georgia, Cambria, Times New Roman, Times`
- **Code/Technical**: `JetBrains Mono`, with fallbacks: `ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, Liberation Mono, Courier New`
...
- **OpenType Features**: `"cswh"` on Fraunces body text, `"ss09"` on Inter buttons/captions

### Hierarchy

| Role | Font | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Inter | 72px (4.50rem) | 400 | 1.10 (tight) | -2.16px | Maximum compression, hero statements |
| Section Heading | Inter | 36px (2.25rem) | 400 | 1.20 (tight) | -0.72px | Feature sections, CTA headlines |
| Sub-heading | Inter | 26px (1.63rem) | 400 | 1.25 (tight) | -0.325px | Card headings, sub-sections |
| Title Small | Inter | 22px (1.38rem) | 400 | 1.30 (tight) | -0.11px | Smaller titles, list headings |
| Body Serif | Fraunces | 19.2px (1.20rem) | 500 | 1.50 | normal | Editorial body with `"cswh"` |
| Body Serif SM | Fraunces | 17.28px (1.08rem) | 400 | 1.35 | normal | Standard body text, descriptions |
| Body Sans | Inter | 16px (1.00rem) | 400 | 1.50 | normal/0.08px | UI body text |
| Button Label | Inter | 14px (0.88rem) | 400 | 1.00 (tight) | normal | Primary button text |
| Button Caption | Inter | 14px (0.88rem) | 400 | 1.50 | 0.14px | Secondary button with `"ss09"` |
| Caption | Inter | 11px (0.69rem) | 400-500 | 1.50 | normal | Small captions, metadata |
...
| Mono Body | JetBrains Mono | 12px (0.75rem) | 400 | 1.67 (relaxed) | normal | Code blocks |
| Mono Small | JetBrains Mono | 11px (0.69rem) | 400 | 1.33 | -0.275px | Inline code, terminal |
...
### Principles
- **Gothic compression for impact**: Inter at display sizes uses -2.16px letter-spacing at 72px, progressively relaxing: -0.72px at 36px, -0.325px at 26px, -0.11px at 22px, normal at 16px and below. The tracking creates a sense of precision engineering.
- **Serif for soul**: Fraunces provides literary warmth. The `"cswh"` feature adds contextual swash alternates that give body text a calligraphic quality.
...
- **Weight restraint**: Inter uses weight 400 almost exclusively, relying on size and tracking for hierarchy rather than weight. System-ui components use 500-700 for functional emphasis.
...
- Links: 14px Inter or system-ui, weight 500
...
- Feature lists using Fraunces serif for readability
...
- **Compressed text, open layout**: Aggressive negative letter-spacing on Inter headlines is balanced by generous surrounding margins. Text is dense; space around it breathes.
...
### Collapsing Strategy
- Hero: 72px Inter → 36px → 26px on smaller screens, maintaining proportional letter-spacing
...
- "Create a hero section on `#f2f1ed` warm cream background. Headline at 72px Inter weight 400, line-height 1.10, letter-spacing -2.16px, color `#26251e`. Subtitle at 17.28px Fraunces weight 400, line-height 1.35, color `rgba(38,37,30,0.55)`. Primary CTA button (`#ebeae5` bg, 8px radius, 10px 14px padding) with hover text shift to `#cf2d56`."
- "Design a card: `#e6e5e0` background, border `1px solid rgba(38,37,30,0.1)`. Radius 8px. Title at 22px Inter weight 400, letter-spacing -0.11px. Body at 17.28px Fraunces weight 400, color `rgba(38,37,30,0.55)`. Use `#f54e00` for link accents."
- "Build a pill tag: `#e6e5e0` background, `rgba(38,37,30,0.6)` text, full-pill radius (9999px), 3px 8px padding, 14px Inter weight 400."
...
- "Design an AI timeline showing four steps: Thinking (`#dfa88f`), Grep (`#9fc9a2`), Read (`#9fbbe0`), Edit (`#c0a8dd`). Each step: 14px system-ui label + 16px Inter description + vertical connecting line in `rgba(38,37,30,0.1)`."

### Iteration Guide
1. Always use warm tones -- `#f2f1ed` background, `#26251e` text, never pure white/black for primary surfaces
2. Letter-spacing scales with font size for Inter: -2.16px at 72px, -0.72px at 36px, -0.325px at 26px, normal at 16px
3. Use `rgba(38, 37, 30, alpha)` as a CSS-compatible fallback for oklab borders
4. Three fonts, three voices: Inter (display/UI), Fraunces (editorial), JetBrains Mono (code)
