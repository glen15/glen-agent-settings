...
### Font Family
- **Display & Body**: `Inter` — geometric sans-serif with soft character. Fallbacks: `Inter Fallback`, system sans-serif
- **Medium**: `Inter` — weight 500 variant for emphasis. Fallbacks: `Inter Fallback`
- **Square**: `Inter` — squared variant for select display contexts. Fallbacks: `Inter Fallback`
...
- **Monospace Body**: `JetBrains Mono` — custom mono companion. Fallbacks: `JetBrains Mono Fallback`

### Hierarchy

| Role | Font | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Inter | 80px | 400 | 1.00 | -2.4px | Maximum compression, hero impact |
| Section Display | Inter | 56px | 400 | 1.20 | -0.56px | Feature section headings |
| Section Heading | Inter | 48px | 400 | 1.20 | -0.48px to -0.96px | Alternate heading weight |
| Feature Heading | Inter | 40px | 400 | 1.10 | -0.4px | Feature block titles |
| Sub-heading Large | Inter | 36px | 400 | 1.15 | -0.72px | Sub-section headers |
| Card Display | Inter | 42px | 400 | 1.00 | 0px | Squared variant for special display |
| Sub-heading | Inter | 32px | 400 | 1.19 | 0px | Content sub-headings |
| Body Heading | Inter | 24px | 400 | 1.20 | -0.72px to 0px | Bold content intros |
| Card Title | Inter | 22px | 500 | 1.14 | 0px | Emphasized card headers |
| Body Large | Inter | 20px | 400 | 1.40 | -0.2px | Primary body text, relaxed |
| Body | Inter | 18px | 400 | 1.30 | -0.18px | Standard body paragraphs |
| Nav/UI | Inter | 16px | 400 | 1.20 | 0px | Navigation links, UI text |
| Button Text | Inter | 16px | 500 | 1.20 | 0px | Button labels |
| Caption | Inter | 14px | 400 | 1.00 | 1.4px | Uppercase labels (transform: uppercase) |
| Small Label | Inter | 12px | 400 | 1.35 | 2.4px | Uppercase micro-labels (transform: uppercase) |
| Micro | Inter | 11px | 400 | 1.20 | 0px | Smallest text elements |
| Code UI | Geist Mono | 16px | 400 | 1.00 | 0px | Terminal/code display |
| Code Body | JetBrains Mono | 16px | 400 | 1.00 | -0.2px | Code content |
| UI Supplement | Inter | 16px | 500 | 1.00 | -0.2px | Specific UI elements |

### Principles
- **Regular weight dominance**: Nearly all text uses weight 400 (Regular) — even headlines. Inter (500) appears only for emphasis moments like card titles and buttons. This creates a remarkably even, calm typographic texture
...
### Navigation
- **Top nav**: Dark background, warm parchment brand text, Inter at 16px for links
...
- Apply Inter (weight 400) for nearly everything — even headlines. Reserve Medium (500) for emphasis only
...
- "Create a hero section on warm dark background with 80px Inter heading in warm parchment (#faf9f6), line-height 1.0, letter-spacing -2.4px, and a dark pill button (#353534, 50px radius, #afaeac text)"
- "Design a feature card with semi-transparent border (rgba(226,226,226,0.35)), 12px radius, warm dark background, Inter heading at 24px, and ash gray (#afaeac) body text at 18px"
- "Build a category label using Inter at 12px, uppercase transform, letter-spacing 2.4px, stone gray (#868584) color — editorial magazine style"
- "Create a testimonial section with warm parchment quotes in Inter 24px, attributed in stone gray (#868584), on dark background with minimal ornamentation"
- "Design a navigation bar with warm dark background, Inter links at 16px in stone gray (#868584), hover to warm parchment (#faf9f6), and a dark pill CTA button (#353534) at the right"
...
3. Check that Inter (400) is the default weight — Medium (500) only for emphasis
